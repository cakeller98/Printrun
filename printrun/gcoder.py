...
class GCode(object):
    ...
    def estimate_duration(self, g=None):
        if (not g):
            g=self.lines
        ...
        #self.estimate = "{0:d} layers, ".format(int(layercount)) + str(datetime.timedelta(seconds = int(totalduration)))
    ...
    def measure(self):
        ...
        ef = 0 # extruder extrusion
        er = 0 # extruder retraction
        rc = 0 # retract count
        ...
        self.ef = ef # total forward extrusion
        self.er = er # total retraction
        self.et = ef+er # total extruder movement
        self.ee = ef-er # total extruded filament
        self.rc = rc # total number of retractions
        self.we = ((self.et/(self.ee))-1.0) * 100.0 # wasted extruder movement
...
def main():
    ...
    print "Total Extruded Filament: %0.02fmm" % gcode.ee
    print "Total Forward Extruder Movement: %0.02fmm" % gcode.ef
    print "Total Retracting Extruder Movement: %0.02fmm" % gcode.er
    print "Total Extruder Movement: %0.02fmm" % gcode.et
    print "Total Retraction Count: %0f retractions" % gcode.rc
    print "Total Retraction Only Percentage: %0.f %%" % gcode.we
